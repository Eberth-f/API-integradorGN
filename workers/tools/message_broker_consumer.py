...
class MessageBrokerConsumer(threading.Thread):
    ...
    Refer:
        - https://stackoverflow.com/questions/36123006/rabbitmq-closes-connection-when-processing-long-running-tasks-and-timeout-settin
    ...
    def __init__(self, amqp_url, queue_name, worker_task, logger):
    ...
    def on_message(self, _unused_channel, basic_deliver, properties, body):
        ...
        """
        self._logger.info(
            'Received message in broker consume #%s from T-%s: %s...',
            basic_deliver.delivery_tag,
            properties.app_id or threading.current_thread().ident,
            body
        )

        # thread = threading.Thread(
        #     target=ConsumerRunner.run,
        #     args=(
        #         self._connection,
        #         self._channel,
        #         basic_deliver.delivery_tag,
        ...
        result = self._worker_task.execute(body)
        if result == True:
            self.acknowledge_message(basic_deliver.delivery_tag)
        # TODO : talvez precise enviar uma mensagem sinalizando que a mensagem não foi processado
    ...
    def stop(self):
        ...
        if not self._closing:
            ...
            self._logger.info('Waiting for threads done...')
            for thread in self._threads:
                thread.join()
            self._logger.info('Threads done...')

#
# class ConsumerRunner(object):
#
#     @classmethod
#     def send_ack_message(cls, channel, delivery_tag, threads, logger):
...
#             logger.warning("Channel is already closed, so we can't ACK this message")
#
#         # todo : remove thread from list
#         # threads
#
#     @classmethod
#     def run(cls, connection, channel, delivery_tag, body, worker_task, threads, logger):
#         """Run task and notify consumer thread when this done"""
#         logger.info('Init task {}'.format(str(worker_task)))
#         worker_task.handle(body)
#         logger.info('End task {}'.format(str(worker_task)))
#         callback = functools.partial(cls.send_ack_message, channel, delivery_tag, threads, logger)
